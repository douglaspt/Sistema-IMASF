...
begin
  ...
  begin
    if Dlg_YesNo('Tem certeza que deseja Excluir permanentemente o Desconto? Não poderá ser recuperado!', self) then
    begin
      ...
      Dlg_Ok('Desconto Excluído', self);
      GeraAuditoria(StrToInt(edt_cod_beneficiario.Text), edt_inscricao_ben.Text,
                             'Excluído cód.desconto ' + edt_cod_descontofolha.Text +
                             ', inscrição ' + edt_inscricao_ben.Text + ', matrícula ' + edt_matricula.Text +
