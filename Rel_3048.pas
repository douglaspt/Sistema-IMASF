...
begin
  ...
  begin
    if Column.Field.Dataset.FieldbyName('nome_tdp').AsString =  'Não Beneficiário' then
...
begin
  ...
  with fdt_principal4.spc_cons_adm_consultaGeralBen do
  begin
    if (FieldByName('nome_tdp').AsString = 'Não Beneficiário')  then
    begin
    ...
    if (FieldByName('titulacartei_ben').AsInteger = 0) and (FieldByName('nome_tdp').AsString <> 'Não Beneficiário')  then
    begin
...
begin
  ...
  begin
    if (FieldByName('nome_tdp').AsString = 'Não Beneficiário')  then
    begin
      Dlg_Alerta('Acesso somente para Beneficiários',self);
