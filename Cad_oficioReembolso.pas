...
begin
  result := true ;
  if (not BAS_ValidarCampo(edt_sequencia_ofr.name, 'A Sequência deve ser preenchida !', VLD_Preenchido )) or
  (not BAS_ValidarCampo(edt_ano_ofr.name, 'O Ano deve ser preenchido !', VLD_Preenchido )) or
  (not BAS_ValidarCampo(edt_cod_beneficiario.name, 'A Inscrição deve ser preenchida !', VLD_Preenchido )) or
  (not BAS_ValidarCampo(edt_dtoficio_ofr.name, 'A Data do Oficio deve ser preenchida !', VLD_Preenchido )) or
  (not BAS_ValidarCampo(edt_obs_ofr.name, 'A Observação deve ser preenchida !', VLD_Preenchido )) then
    Exit;
  result := false ;
end;
...
begin
  ...
  FPesquisaTit  := 'Código, Sequência, Ano, Inscrição, Nome do Beneficiário, Dt. Oficio, Observação ' ;
