...
begin
  ...
  GeraAuditoria(StrToInt(edt_cod_beneficiario.text), ed2_inscricao_ben.Text,'Cancelado emissão carteirinha Inscrição ' + ed2_inscricao_ben.Text, '', 0);
  PageControl1.TabOrder:= 0;
  ChaveExp:= 999999999;
end;
...
begin
  ...
  (not BAS_ValidarCampo(edt_carteira.name, 'É necessario selecionar um beneficiário !', VLD_Preenchido )) then
    Exit;
  ...
  GeraAuditoria(StrToInt(edt_cod_beneficiario.text), edt_inscricao_ben.Text,'Incluído carteirinha Inscrição ' + edt_inscricao_ben.Text, '', 0);
  PageControl1.TabOrder:= 0;
  ChaveExp:= 999999999;
end;
