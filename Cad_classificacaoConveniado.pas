...
begin
  ...
  FPesquisaTit  := 'Código, Descrição, Dotação' ;
  inherited;
end;
...
begin
  result := true ;
  if (not BAS_ValidarCampo(edt_cod_classificacaoConveniado.name, 'O Código deve ser preenchido !', VLD_Preenchido )) or
  (not BAS_ValidarCampo(edt_descricao_ccv.name, 'A Descrição deve ser preenchida !', VLD_Preenchido )) or
  (not BAS_ValidarCampo(edt_dotacao_cvv.name, 'A Dotação deve ser preenchida !', VLD_Preenchido )) then
    Exit;
  result := false ;
end;
