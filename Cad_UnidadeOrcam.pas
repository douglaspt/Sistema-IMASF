...
begin
  result := true ;
  if (not BAS_ValidarCampo(edt_cod_unidadeOrcam.name, 'O Código Unidade deve ser preenchido !', VLD_Preenchido )) or
  (not BAS_ValidarCampo(edt_descUnidade_uno.name, 'O campo Descrição deve ser preenchido !', VLD_Preenchido )) or
  (not BAS_ValidarCampo(edt_exercicio_uno.name, 'O Ano Exercício deve ser preenchido !', VLD_Preenchido )) then
    Exit;
  result := false ;
end;
...
begin
  inherited;
  if not BAS_ValidarCampo(cbx_cod_dotacaoBase.name, 'É necessário preencher a Dotação', VLD_Preenchido) then
    exit ;
  if not BAS_ValidarCampo(edt_valor_dtc.Name, 'É necessário preencher o Valor', VLD_Preenchido) then
    exit ;
